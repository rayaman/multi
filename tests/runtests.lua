...
function testing.False(value)
	if value ~= false then
		thread.kill("Expected value to be false")
	end
end

function testing.True(value)
	if value ~= true then
		thread.kill("Expected value to be true")
	end
end

function testing.IsNil(value)
	if value ~= nil then
		thread.kill("Expected nil, but got: " .. tostring(value) .."")
	end
end

function testing.NotNil(value)
	if value == nil then
		thread.kill("Expected value not to be nil.")
	end
end

function testing.Equal(expected, actual)
	if expected ~= actual then
		thread.kill("Not equal: \n" ..
		"expected: " .. tostring(expected) .. "\n" ..
		"actual  : " .. tostring(actual) .. "\n")
	end
end

function testing.NotEqual(expected, actual)
	if expected == actual then
		thread.kill("Should not be: values are equal")
	end
end
...
function ListFiles(path)
	...
	if getOS() == "Windows" then
		for dir in io.popen([[dir /A-d "]] .. path .. [[" /b]]):lines() do table.insert(files,dir) end
	else
		for dir in io.popen([[ls -pa "]] .. path .. [[" | grep -v /]]):lines() do table.insert(files,dir) end
	end
	return files
end
...
runTest = thread:newFunction(function()
	...
	-- end
	units = ListFiles("tests/units")
	for _, file in pairs(units) do
		multi.print("Running tests in: " .. file)
		unit = loadfile("tests/units/" .. file)()
		function Test(...)
			unit.Test(...)
			multi.success(file)
		end
		func = thread:newFunction(Test, true)
		func(multi:newProcessor(file), thread, testing)
	end
	if not love then
		local ec = 0
